...
import cn.xcom.banjing.adapter.LocalImgGridAdapter;
...
import cn.xcom.banjing.utils.VolleyRequest;
import cn.xcom.banjing.view.NoScrollGridView;
import cn.xcom.banjing.view.WheelView;
...
public class HelpMeActivity extends BaseActivity implements View.OnClickListener, OnGetGeoCoderResultListener {
    ...
    private QRecordAudioView qRecordAudioView;
    private PlayRecordAudioView playRecordAudioView;
   ...
   //录音语音Dialog,录音时长
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.rl_photo:
                ...
                break;
            case R.id.rl_voice:
                if (PermissionUtils.hasSelfPermissions(HelpMeActivity.this, PERMISSION_SHOWVOICE)) {
                    record();
                    return;
                } else {
                    ActivityCompat.requestPermissions(HelpMeActivity.this, PERMISSION_SHOWVOICE, REQUEST_SHOWVOICE);
                }
               break;
        }
    ...
    public void record(){
        View view = LayoutInflater.from(HelpMeActivity.this).inflate(R.layout.dialog_qrecord_audio_view,null);
        qRecordAudioView= (QRecordAudioView) view.findViewById(R.id.dialog_q_record);
        playRecordAudioView= (PlayRecordAudioView) view.findViewById(R.id.dialog_q_play_record);
        dialog=new AlertDialog.Builder(HelpMeActivity.this).setView(view).create();
        dialog.show();
        playRecordAudioView.setRecordInterfaceListener(playRecordListener);
        qRecordAudioView.setRecordInterfaceListener(new RecordInterfaceListener() {
            @Override
            ...
            public void OnPlayRecordSeleted(int Duration, String filePath) {
                recordPath = filePath;
                duration=Duration;
                qRecordAudioView.setVisibility(View.GONE);
                playRecordAudioView.setVisibility(View.VISIBLE);
                playRecordAudioView.setRecordTime(new Long(Duration));

            }

            @Override
            public void OnRecordFinish(int Duration, String filepath, String RecordName) {
               // sendSound(filePath, recordName, Duration);
                recordPath=filepath;
                soundView.setVisibility(View.VISIBLE);
                soundView.init(filepath, Duration);
                dialog.dismiss();
            }
        });
    ...
    private PlayRecordListener playRecordListener = new PlayRecordListener() {
        ...
        public void OnSendRecordSeleted() {
            soundView.setVisibility(View.VISIBLE);
            soundView.init(recordPath, duration);
            dialog.dismiss();
            ToastUtils.showToast(HelpMeActivity.this, "发送");
        }

        @Override
        public void OnCancleRecordSeleted() {
            ToastUtils.showToast(HelpMeActivity.this, "取消");
            mediaPlayer.stop();
            qRecordAudioView.setVisibility(View.VISIBLE);
            playRecordAudioView.setVisibility(View.GONE);
        }

        @Override
        public void OnStartRecord() {
            ToastUtils.showToast(HelpMeActivity.this, "播放");
            try {
                mediaPlayer.reset();
        ...
        public void OnStopRecord() {
            ToastUtils.showToast(HelpMeActivity.this, "暂停");
        }
    };
    @Override
...
//            request.putValue("soundtime", soundView.getSoundTime() + "");
        ...
        LogUtils.e("ABC---request", "picurl" + StringJoint.arrayJointchar(nameList, ",") + "\n" + "userid" + userInfo.getUserId() + "\n" + "title" + et_content.getText().toString()
    ...
    public void onRequestPermissionsResult(int permsRequestCode, String[] permissions, int[] grantResults) {
        switch (permsRequestCode) {
            ...
            case REQUEST_SHOWVOICE:
                if (PermissionUtils.getTargetSdkVersion(HelpMeActivity.this) < 23 && !PermissionUtils.hasSelfPermissions(HelpMeActivity.this, PERMISSION_SHOWVOICE)) {
                    return;
                }
