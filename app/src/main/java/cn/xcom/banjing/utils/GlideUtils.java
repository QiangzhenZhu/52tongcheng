...
public class GlideUtils {
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).error(R.mipmap.ic_logo).into(mImageView);
    }
    //加载圆角
...
//        Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).into(mImageView);
        Glide.with(mContext).load(path).error(R.mipmap.ic_logo).transform(new GlideRoundTransform(mContext, 10)).into(mImageView);
    }
    //显示头像
    public static void showAvatar(Context mContext,String path,ImageView mImageView){
        Glide.with(mContext).load(path).error(R.drawable.rp_avatar).into(mImageView);
//    //默认加载
//    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
//        Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).into(mImageView);
//    }
